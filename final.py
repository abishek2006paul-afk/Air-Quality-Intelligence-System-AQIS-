...
def show_home():
    st.markdown(
        ...
        "<h1>🌍 Air Quality Intelligence System (AQIS)</h1>"
...
# ─────────────────────────────────────────────
    ...
    if clf is not None:
        st.sidebar.title("🧭 AQIS")
        choice = st.sidebar.radio(
            "Go To:",
            ["Home", "Prediction", "Dashboard", "Municipal Team"],
            index=["Home", "Prediction", "Dashboard", "Municipal Team"].index(st.session_state["page"])
                  if st.session_state["page"] in ["Home", "Prediction", "Dashboard", "Municipal Team"] else 0
        )
        ...
        if st.sidebar.button("🔓 Logout"):
            st.session_state["logged_in"] = False
            st.rerun()
# # ── System Monitor (for 8GB RAM / i5-12450H laptop) ──
#         st.sidebar.divider()
#         # st.sidebar.caption("💻 System Monitor")  # hidden from UI
#         try:
#             import psutil
...
#             pct   = ram.percent
#             ram_color = "🟢" if pct < 70 else ("🟡" if pct < 85 else "🔴")
#             cpu_color = "🟢" if cpu  < 60 else ("🟡" if cpu  < 85 else "🔴")
#             st.sidebar.markdown(
#                 f"{ram_color} **RAM:** {used:.1f} / {total:.1f} GB &nbsp; `{pct}%`  \n"
#                 f"{cpu_color} **CPU:** `{cpu:.0f}%`  \n"
#                 f"🖥️ **Cores used:** 4 / 8 (i5-12450H)"
#             )
#             if pct > 85:
#                 st.sidebar.warning("⚠️ RAM high! Close Chrome tabs / other apps.")
#         except ImportError:
#             pass  # hidden: "Tip: pip install psutil for live RAM/CPU stats"
